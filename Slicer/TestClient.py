...
class SOFAClient():
    ...
    def connect_client(self, number_of_attempt = 10, wait_time = 0.1):
        it = 0
        while(not self.__internal_connect_to_client() and it < number_of_attempt):
    ...
    def __internal_connect_to_client(self):
        try:
            self.connection = rpyc.connect("localhost", 18813, config = {'allow_public_attrs': True, "allow_all_attrs": True, "allow_pickle": True})
